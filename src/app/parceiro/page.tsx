...
export default function ParceiroPage() {
  ...
  const [pSimPermLink, setPSimPermLink] = useState("");
  const [pSimPermLinkCopied, setPSimPermLinkCopied] = useState(false);
  const [pSimPermLinkLoading, setPSimPermLinkLoading] = useState(false);
        ...
        return (
          <div className="max-w-3xl mx-auto px-4 sm:px-8 py-8">
            ...
            <div className="bg-white border border-[#e2e2e2] p-6 space-y-6">
              ...
              {/* Gerar orçamento permanente */}
              <button
                disabled={!canGeneratePSim || pSimPermLinkLoading}
                onClick={async () => {
                  setPSimPermLinkLoading(true);
                  setPSimPermLink("");
                  setPSimPermLinkCopied(false);
                  try {
                    const discFactor = pDiscountFactor;
                    const spaces = pAllSpaces.map(sp => {
                      const prod = sp.product;
                      const linePrice = NORMAL_PRICES[sp.productCode.startsWith("ORB") ? (prod?.linha ?? "Classic") : "Classic"];
                      const pricePerPlate = Math.round(linePrice * discFactor);
                      return {
                        spaceName: sp.spaceName,
                        productCode: sp.productCode,
                        productName: prod?.name ?? sp.productCode,
                        productImg: prod?.img ?? "",
                        linha: prod?.linha ?? "Classic",
                        plates: sp.plates,
                        area: sp.area,
                        pricePerPlate,
                        total: sp.material,
                    ...
                    const materialTotal = pAllSpaces.reduce((s, sp) => s + sp.plates * NORMAL_PRICES[sp.product?.linha ?? "Classic"], 0);
                    const materialDiscounted = pGrandMaterial;
                    const res = await fetch("/api/quotes", {
                      ...
                      body: JSON.stringify({
                        ...
                        total_area_m2: pAllSpaces.reduce((s, sp) => s + sp.area, 0),
                        material_total: materialTotal,
                        material_discounted: materialDiscounted,
                      }),
                    });
                    const { slug } = await res.json();
                    const origin = typeof window !== "undefined" ? window.location.origin : "https://orbitalrevestimentos.com.br";
                    setPSimPermLink(`${origin}/orcamento/${slug}`);
                    setPSimLink(buildPSimLink());
                    setPSimLinkCopied(false);
                  } finally {
                    setPSimPermLinkLoading(false);
                    setTimeout(() => document.getElementById("psim-link")?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
                  }
                }}
                className="w-full py-3 text-xs tracking-[0.12em] uppercase font-bold font-[var(--font-inter)] bg-[#002045] text-white hover:bg-[#1a365d] transition-colors disabled:opacity-40 flex items-center justify-center gap-2"
              >
                {pSimPermLinkLoading ? (
            ...
            {pSimPermLink && (
              <div className="mt-6 space-y-4">
                {/* Permanent link card */}
                <div className="bg-[#002045] p-6 space-y-4">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-white/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
                    </div>
                    <div>
                      <p className="text-white text-[10px] tracking-[0.18em] uppercase font-bold font-[var(--font-inter)] mb-0.5">Orçamento permanente</p>
                      <p className="text-white/60 text-[10px] font-[var(--font-inter)]">Válido por 7 dias · com imagens e resumo do projeto</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <input readOnly value={pSimPermLink} onClick={e => (e.target as HTMLInputElement).select()}
                      className="flex-1 bg-white/10 border border-white/20 px-3 py-2 text-xs font-[var(--font-inter)] text-white focus:outline-none" />
                    <button onClick={() => { navigator.clipboard.writeText(pSimPermLink); setPSimPermLinkCopied(true); setTimeout(() => setPSimPermLinkCopied(false), 2000); }}
                      className="px-4 py-2 text-[10px] tracking-[0.1em] uppercase font-bold font-[var(--font-inter)] border border-white/40 text-white hover:bg-white hover:text-[#002045] transition-colors whitespace-nowrap">
                      {pSimPermLinkCopied ? "Copiado ✓" : "Copiar"}
                    </button>
                  </div>
                  {/* WhatsApp with permanent link */}
                  {(() => {
                    const waPermText = encodeURIComponent(
                      [`Olá! Preparei um orçamento exclusivo do seu projeto com PFB Orbital:`, ``, pSimPermLink, ``,
                       ...pAllSpaces.map((sp, i) => `${i + 1}. ${sp.spaceName} — ${sp.product?.name ?? sp.productCode} · ${sp.plates} placas`),
                       ...(pAllSpaces.length > 1 ? [``, `Total do projeto: R$${fmtParceiro(pGrandMaterial)}`] : []),
                       ``, `O link tem fotos dos produtos e é válido por 7 dias.`].join("\n")
                    );
                    return (
                      <a href={`https://wa.me/?text=${waPermText}`} target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2.5 w-full bg-[#25d366] text-white text-xs tracking-[0.12em] uppercase font-bold font-[var(--font-inter)] py-3 hover:bg-[#1ebe5d] transition-colors">
                ...
                {/* Also: quick simulator link */}
                <div className="bg-white border border-[#e2e2e2] p-4 space-y-3">
                  <p className="text-[#74777f] text-[9px] tracking-[0.18em] uppercase font-[var(--font-inter)] font-bold">Ou use o link do simulador interativo</p>
                  <div className="flex gap-2">
                  ...
                  <p className="text-[#b0b4bb] text-[10px] font-[var(--font-inter)]">
                    {pAllSpaces.length > 1 ? `${pAllSpaces.length} ambientes pré-configurados. O cliente preenche seus dados.` : `Espaço e produto pré-configurados com seu cupom.`}
                  </p>
                </div>
              </div>
            )}
